...
$prestamoPendiente=GetAllUsuarioCronograma($_POST["meses_pendientes"], 1);
    ...
    while ($fila>$contador){
        ...
        for ($i=0; $i < 15; $i++) {
            if ($i==7) {
                ...
                if ($prestamoPendiente[$contador][$i]==0) {
                    echo "<td class='contenido' style='border:1px solid black;'>Ninguno</td>";
                }
                else if ($prestamoPendiente[$contador][$i]==1) {
                ...
                echo "<td class='contenido' style='border:1px solid black;'>".$prestamoPendiente[$contador][$i]."</td>";
            }
...
$dompdf->render();
$dompdf->stream("pendiente.pdf", array('Attachment' => false));
