...
$prestamoDevuelto=GetAllUsuarioCronograma($_POST["meses_pendientes"], 2);
    ...
    while ($fila>$contador){
        ...
        for ($i=0; $i < 15; $i++) {
            if ($i==0) {
                ...
                if ($prestamoDevuelto[$contador][$i]==0) {
                    echo "<td class='contenido' style='border:1px solid black;'>Ninguno</td>";
                }
                else if ($prestamoDevuelto[$contador][$i]==1) {
                ...
                echo "<td class='contenido' style='border:1px solid black;'>".$prestamoDevuelto[$contador][$i]."</td>";
            }
...
$dompdf->render();
$dompdf->stream("devuelto.pdf", array('Attachment' => false));
