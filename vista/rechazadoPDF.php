...
$prestamoRechazado=GetAllUsuarioCronograma($_POST["meses_pendientes"],0);
    ...
    while ($fila>$contador){
        ...
        for ($i=0; $i < 15; $i++) {
            if ($i==7) {
                ...
                if ($prestamoRechazado[$contador][$i]==0) {
                    echo "<td class='contenido' style='border:1px solid black;'>Ninguno</td>";
                }
                else if ($prestamoRechazado[$contador][$i]==1) {
                ...
                echo "<td class='contenido' style='border:1px solid black;'>".$prestamoRechazado[$contador][$i]."</td>";
            }
...
$dompdf->render();
$dompdf->stream("rechazado.pdf", array('Attachment' => false));
